...
- 网页笔记按页面 URL 保存在已授权文件夹的 `web-page-notes.json` 中，打开同一页面时会自动显示。
- 可在扩展弹窗中点击“设置保存文件夹”，选择本地文件夹并授权读写。没有读写权限时，新增、编辑或删除笔记会直接失败并提示重新授权。
- 卸载扩展不会删除已写入本地文件夹的 `web-page-notes.json`。
...
5. 如果无法保存笔记，打开扩展的“设置保存文件夹”，重新选择文件夹并点“授权并写入文件”。
